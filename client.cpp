...
void checkAppendSendRecieve(vector<string> &arg, string &command, string ip,
  ...
  void DownloadChunkRange(
    ...
    if (sock < 0) {
      cerr << "❌ Failed to create socket to peer " << peer.ip << ":"
           << peer.port << endl;
      return;
    }
  ...
  void DownloadFromClient(int clientPort, string destinationPath,
                          string fileName, const vector<string> &chunkHashes,
                          const string &expectedCompleteHash)
...
//   Function to receive data from the client
  void RecieveFile(int clientSocket, string destinationPath, string
  fileName,
                   const vector<string> &chunkHashes,
                   const string &expectedCompleteHash) {
    string fullPath = destinationPath + "/" + fileName;
    ...
    int bytesRead = recv(clientSocket, &totalFileSize,
    sizeof(totalFileSize), 0); cout << "Total file size to receive: " <<
    totalFileSize << " bytes" << endl; if (bytesRead !=
    ...
    while (totalReceived < totalFileSize) {
      ...
      cout << "Local hash for chunk " << chunkIndex << ": " << localHash <<
      endl; if (chunkIndex >= chunkHashes.size()) {
      ...
      if (localHash != chunkHashes[chunkIndex]) {
        ...
        cout << "✔️ Chunk " << (chunkIndex + 1) << " hash verified." <<
        endl;
      }
      ...
      cout << "Received " << totalReceived << " bytes out of " <<
      totalFileSize
           << " bytes." << endl;
    }
    ...
    cout << "Expected complete file hash: " << expectedCompleteHash <<
    endl; if (finalHash != expectedCompleteHash) {
      cerr << "❌ Final file hash mismatch! File may be corrupted." <<
      endl;
    } else {
      cout << "✅ Final file hash verified successfully." << endl;
    }
  ...
  void DownloadHandler(string clientIP, string port) {
    ...
    while (true) {
      ...
      int peerSocket = accept(listeningSocket,
  ...
  void FindFileMetadata(string filePath, string & command) {
    ...
    cout << "Hash of complete file while storing: "
         << metadata.HashOfCompleteFile << endl;
  ...
  string DeserializePeerStats(const PeerStats &peer) {
    ...
    string message = "update_peer_stats " + peer.ip + " " +
                     to_string(peer.port) + " " + to_string(peer.score) + " " +
